...
# Singleton class to cache Keras model loading
class KerasModelManager:
    _instance = None
    _model = None

    @classmethod
    def get_model(cls, weights_path):
        if cls._model is None:
            # Build and load Keras model
            cls._model = create_pure_mold_vit_model(image_size=256)
            # Dry run to initialize variables
            dummy_input = tf.zeros((1, 256, 256, 3))
            _ = cls._model(dummy_input, training=False)
            # Load weights
